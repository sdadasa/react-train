...
export default class Nac extends React.Component{
    constructor(props){
        super(props);
        this.state = { qingdan: [],monitor:false,resp:''};
    } 
    clickHanppen=(API)=>{
        this.setState({resp:API,monitor:true})
    }
    ...
    async componentDidUpdate(){
        const res=await axios.get(this.state.resp)
    ...
    render(){
        ...
        if (this.state.monitor === true) {
            ...
            return(<div style={ {justifyContent: 'space-around', maxWidth: '100%', margin: '0 auto', paddingLeft: '20%', fontSize: '18px', fontWeight: 'bold'}}>
            <ul style={{display: 'flex', flexDirection: 'row', listStyle: 'none', paddingLeft: '30%'}}>
